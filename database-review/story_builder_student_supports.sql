...
create table public.story_builder_student_supports (
  student_id uuid not null,
  owner_user_id uuid not null,
  target_key text not null,
  support_keys text[] not null default '{}'::text[],
  updated_at timestamptz not null default now(),
  constraint story_builder_student_supports_pkey primary key (student_id),
  constraint story_builder_student_supports_student_owner_fkey
    foreign key (student_id, owner_user_id)
    references public.students (id, owner_user_id)
    on delete cascade,
  constraint story_builder_student_supports_target_key_check
    check (target_key in ('story_organization', 'connections_cohesion', 'cause_effect', 'sentence_formulation', 'elaboration', 'perspective_internal_state', 'vocabulary_precision')),
  constraint story_builder_student_supports_keys_check
    check (
      case target_key
...
create index story_builder_student_supports_owner_idx
  on public.story_builder_student_supports (owner_user_id, student_id);

alter table public.story_builder_student_supports enable row level security;

create policy story_builder_student_supports_educator_select
  on public.story_builder_student_supports
  for select to authenticated
  using ((select auth.uid()) = owner_user_id);

revoke all on table public.story_builder_student_supports from anon;
revoke insert, update, delete on table public.story_builder_student_supports from authenticated;
grant select on table public.story_builder_student_supports to authenticated;

create or replace function public.set_story_builder_student_supports(
  p_student_id uuid,
  p_target_key text,
  p_support_keys text[]
)
returns table (target_key text, support_keys text[])
...
begin
  ...
  if p_student_id is null
     ...
     or p_target_key not in ('story_organization', 'connections_cohesion', 'cause_effect', 'sentence_formulation', 'elaboration', 'perspective_internal_state', 'vocabulary_precision')
  ...
  if pg_catalog.cardinality(p_support_keys) <> (
    select count(distinct support_key)
    from pg_catalog.unnest(p_support_keys) as support_key
  ) then
    raise exception 'duplicate support key';
  end if;
  if not exists (
    select 1 from public.students as s
    where s.id = p_student_id and s.owner_user_id = v_owner_user_id and s.archived_at is null
  ) then
  ...
  if not exists (
    ...
    where pe.owner_user_id = v_owner_user_id
      and pe.product_key = 'first-volo-story-builder'
      and pe.status = 'active'
      and pe.starts_at is not null and pe.expires_at is not null
      and pe.starts_at <= pg_catalog.now() and pe.expires_at > pg_catalog.now()
  ) then
    raise exception 'product access not active';
  end if;

  delete from public.story_builder_student_supports as ss
  where ss.student_id = p_student_id and ss.owner_user_id = v_owner_user_id;

  if cardinality(p_support_keys) > 0 then
    insert into public.story_builder_student_supports (student_id, owner_user_id, target_key, support_keys, updated_at)
    values (p_student_id, v_owner_user_id, p_target_key, p_support_keys, pg_catalog.now());
  end if;

  return query
    select ss.target_key, ss.support_keys
    from public.story_builder_student_supports as ss
    where ss.student_id = p_student_id and ss.owner_user_id = v_owner_user_id;
end;
$$;

create or replace function public.get_story_builder_student_supports_for_educator(p_student_id uuid)
returns table (target_key text, support_keys text[])
language plpgsql security definer set search_path = ''
as $$
declare
...
create or replace function public.get_story_builder_student_supports()
returns table (target_key text, support_keys text[])
language plpgsql security definer set search_path = ''
as $$
begin
...
revoke all on function public.set_story_builder_student_supports(uuid, text, text[]) from public, anon;
...
grant execute on function public.set_story_builder_student_supports(uuid, text, text[]) to authenticated;
